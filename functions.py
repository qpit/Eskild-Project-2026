...
def crystal_length(L_0, T, T_0, a_z):
    """Calculate the thermally expanded crystal length."""
    return L_0 * (1 + a_z * (T - T_0))
...
def field_envelope(tau, rho, QPM, L, Delta_r, Delta_l, n_2w, omega_2w, n_w, omega_w, phi_l=0, phi_r=0):
    ...
    numerator = 1j * tau * (field_left  * np.exp(-1j * phase_mismatch_term*Delta_l) * np.exp(1j*phi_l) + 
                        field_right * np.exp(-1j * phase_mismatch_term*Delta_r) * np.exp(1j*phi_r))
    denominator = 1 - rho
...
def relative_phase_RT(Delta_l, Delta_r, D, L, n_2w, omega_2, n_w, omega_w, phi_l, phi_r):
    ...
    L_tot = Delta_l + L + Delta_r
    QPM = phase_mismatch(n_2w, omega_2, n_w, omega_w)
    phi_rt = 2*(L_tot * QPM + phi_l + phi_r)
    return phi_rt
...
def double_resonance_temperatures(T, Delta_l, Delta_r, D, L, n_2w, omega_2, n_w, omega_w, phi_l, phi_r):
    ...
    for i in sign_changes:
        # Linear interpolation to find exact temperature where val crosses zero
        # Between points i and i+1, find where val = 0
        val_i = val[i]
        val_i1 = val[i+1]
        T_i = T[i]
        T_i1 = T[i+1]
        
        # Linear interpolation: T_zero = T_i + (T_i1 - T_i) * (-val_i) / (val_i1 - val_i)
        if val_i1 != val_i:  # Avoid division by zero
            T_interp = T_i - val_i * (T_i1 - T_i) / (val_i1 - val_i)
            # Find the closest index to the interpolated temperature
            closest_idx = np.argmin(np.abs(T - T_interp))
            T_indices.append(closest_idx)
        else:
            # Fallback to choosing the point closer to zero
            if np.abs(val_i) < np.abs(val_i1):
                T_indices.append(i)
            else:
...
def matched_peaks(temperature, peaks_in_SHG, T_indi, tol=0.5):
    ...
    """
    tol = 1  # Tolerance in °K for matching peaks
...
def envelope_and_T(tau, rho, QPM, L, Delta_r, Delta_l, n_2w, omega_2w, n_w, omega_w, D, temperature, phi_l=0, phi_r=0,a_z=1e-5):
    """
    ...
    envelope : array
        The field envelope.
    T_indices : array
        Indices of resonance temperatures (peaks in the envelope).
    """
    length = crystal_length(L, temperature, 25+273.15, a_z)  # Example thermal expansion coefficient
    envelope = field_envelope(tau, rho, QPM, length, Delta_r, Delta_l, n_2w, omega_2w, n_w, omega_w, phi_l, phi_r)
    # Calculate resonance temperatures based on the double resonance phase condition
    T_indi = double_resonance_temperatures(temperature, Delta_l, Delta_r, D, length, n_2w, omega_2w, n_w, omega_w, phi_l, phi_r)
